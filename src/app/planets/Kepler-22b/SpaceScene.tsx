...
import createStars from '../../../components/StarField';
import getSoilLayer from '../../../components/soil/SoilField';
import getWaterLayer from '../../../components/water/WaterField';
import getCloudLayer from '../../../components/cloud/CloudField';
import { OrbitControls } from 'three/addons/controls/OrbitControls.js';
import getFresnelMat  from '../../../components/fresnel/fresnelField';
...
const SpaceScene: React.FC = () => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && containerRef.current) {
      ...
      // Soil Layer
      const soilMesh = getSoilLayer(2);

      //Water Layer
      const waterMesh = getWaterLayer({
        index: 6,
        newColor: 0x008080,
        multiplier:1.159,       // vibrant ocean blue
        newOpacity: 0.7,          // slightly transparent water
        newShininess: 150,        // mild gloss
        newReflectivity: 0.8      // decent reflection
      });

      //Cloud Layer
      const {cloudMaterial_, cloudQuad} = getCloudLayer({camera_:camera,cloudInnerRadius:3.0,cloudOuterRadius:3.2, percentageCloud:0.45,currentPreset_:"random", cloudColor:new THREE.Color(1,1,1)});
