...
export default function Sim({
  ...
  const derivedParams = useMemo(() => {
    ...
    const orbitalPeriod =
      2 * Math.PI * Math.sqrt(Math.pow(distanceMeters, 3) / (G * sunMassKg)); // seconds
    ...
    const temperature = Math.pow(
      (luminosity * (1 - albedo)) /
        (16 * Math.PI * Math.pow(distanceMeters, 2) * STEFAN_BOLTZMANN),
      0.25
    ); // Kelvin
  ...
  const esi = useMemo(() => {
    ...
    const radiusESI = Math.max(
      0,
      1 - Math.abs((radiusEarth - 1) / (radiusEarth + 1))
    );
    const densityESI = Math.max(0, 1 - Math.abs((density - 1) / (density + 1)));
    const escapeESI = Math.max(
      0,
      1 - Math.abs((escapeVelocity - 1) / (escapeVelocity + 1))
    );
    const tempESI = Math.max(
      0,
      1 - Math.abs((temperature - 1) / (temperature + 1))
    );
    const periodESI = Math.max(
      0,
      1 - Math.abs((orbitalPeriod - 1) / (orbitalPeriod + 1))
    );
    const pressureESI = Math.max(
      0,
      1 - Math.abs((atmosphericPressure - 1) / (atmosphericPressure + 1))
    );
  ...
  const getHabitabilityClass = (esi: number) => {
    if (esi >= 0.8)
      return {
        class: "Highly Habitable",
        color: "bg-green-500",
        textColor: "text-green-700",
      };
    if (esi >= 0.6)
      return {
        class: "Moderately Habitable",
        color: "bg-yellow-500",
        textColor: "text-yellow-700",
      };
    if (esi >= 0.4)
      return {
        class: "Marginally Habitable",
        color: "bg-orange-500",
    ...
    <TooltipProvider>
      <div className="relative w-full h-screen overflow-hidden bg-black">
        ...
        <div className="flex flex-col gap-2">
          <div className="fixed top-4 left-4 w-80 bg-black/20 backdrop-blur-md rounded-xl border border-white/10 p-6 ring-1 ring-white/50 shadow-[0_0_10px_rgba(255,255,255,0.6)]">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Tooltip>
                      <TooltipTrigger>
                        ...
                        <TooltipContent className="max-w-xs w-60 whitespace-normal break-words">
                          <p>
                            R⊕ = Earth Radii. Affects gravity, density, and
                    ...
                    {(planetRadiusSlider <= 1 || planetRadiusSlider >= 10) && (
                      <Tooltip>
                        ...
                        <TooltipContent className="max-w-xs w-60 whitespace-normal break-words">
                          <p>
                            Cannot render the 3D model accurately at extreme
                            values
                          </p>
                        </TooltipContent>
                      </Tooltip>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Tooltip>
                      <TooltipTrigger>
                        ...
                        <TooltipContent className="max-w-xs w-60 whitespace-normal break-words">
                          <p>
                            M⊕ = Earth Masses. Determines escape velocity and
                            atmospheric retention.
                          </p>
                        </TooltipContent>
                      </TooltipTrigger>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Tooltip>
                      <TooltipTrigger>
                        ...
                        <TooltipContent className="max-w-xs w-60 whitespace-normal break-words">
                          <p>
                            M☉ = Solar Masses. Affects luminosity and orbital
                            dynamics.
                          </p>
                        </TooltipContent>
                      </TooltipTrigger>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Tooltip>
                      <TooltipTrigger>
                        ...
                        <TooltipContent className="max-w-xs w-60 whitespace-normal break-words">
                          <p>
                            Orbital distance from star. Determines temperature
                            and habitability zone.
                          </p>
                        </TooltipContent>
                      </TooltipTrigger>
                    </Tooltip>
                    ...
                    {/* Warning icon for extreme values */}
                    {(sunDistanceSlider <= 400 ||
                      sunDistanceSlider >= 1100) && (
                      <Tooltip>
                        <TooltipTrigger>
                          <AlertTriangle className="h-4 w-4 text-yellow-500" />
                        </TooltipTrigger>
          ...
          <div className="cursor-pointer fixed top-85 left-4 z-50 rounded-2xl hover:scale-105 transition-transform duration-200 justify-center text-center items-center backdrop-blur-2xl ring-1 ring-white/50 shadow-[0_0_10px_rgba(255,255,255,0.6)]">
            <Button
              className="bg-black/20 backdrop-blur-md border border-white/10 text-white hover:text-black hover:bg-white/80 transition-colors duration-200 rounded-2xl"
              onClick={handleClick}
            >
              <RotateCcw
                size={16}
                className={`mr-2 ${spinning ? "spin-animation" : ""}`}
              />
              Reset
            </Button>
          </div>
          ...
          <Card className="bg-black/20 backdrop-blur-md border border-white/10 text-white ring-1 ring-white/50 shadow-[0_0_10px_rgba(255,255,255,0.6)]">
            <CardHeader className="text-center pb-4">
              ...
              <div className="flex justify-center">
                <Badge
                  className={`${habitabilityClass.color} text-white text-sm px-3 py-1`}
                >
            ...
            <CardContent className="space-y-6">
                ...
                <div className="flex items-center gap-2  justify-center">
                  ...
                  <Tooltip>
                    <TooltipTrigger>
                      <Info className="h-3 w-3 text-gray-400" />
                      <TooltipContent className="max-w-xs w-60 whitespace-normal break-words">
                        Scale from 0-1 comparing planet to Earth. Values 0.8-1.0
                        indicate Earth-like conditions. Based on radius,
                        density, escape velocity, and temperature - with
                        temperature having the strongest influence.
                      </TooltipContent>
                    </TooltipTrigger>
                  </Tooltip>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Tooltip>
                        <TooltipTrigger>
                          <Info className="h-3 w-3 text-gray-400" />
                          <TooltipContent className="max-w-xs w-60 whitespace-normal break-words">
                            Mass per unit volume (g/cm³). Increases with mass,
                            decreases with radius cubed. Earth&apos;s density:
                            5.5 g/cm³. Higher density = rockier composition,
                            better atmospheric retention.
                          </TooltipContent>
                        </TooltipTrigger>
                      </Tooltip>
                    </div>
                  ...
                  <div>
                      ...
                      <Tooltip>
                        <TooltipTrigger>
                          ...
                          <TooltipContent className="max-w-xs w-60 whitespace-normal break-words">
                            ...
                            (km/s). Formula: √(2GM/R). Increases with mass,
                            decreases with radius.Earth&apos;s: 11.2 km/s.
                            Higher values retain atmospheres better.
                          </TooltipContent>
                        </TooltipTrigger>
                      </Tooltip>
                    </div>
                    ...
                    <p className="font-mono text-blue-400">
                      <AnimatedNumber
                        value={derivedParams.escapeVelocity}
                        decimals={2}
                        suffix=" v⊕"
                      />
                    </p>
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <Tooltip>
                        <TooltipTrigger>
                          <Info className="h-3 w-3 text-gray-400" />
                          <TooltipContent className="max-w-xs w-60 whitespace-normal break-words">
                            Time to complete one orbit around the star.
                            Increases with orbital distance cubed (Kepler&apos;s
                            3rd Law). Affects seasonal patterns and climate
                            stability.
                          </TooltipContent>
                        </TooltipTrigger>
                      </Tooltip>
                    ...
                    <p className="font-mono text-blue-400">
                      <AnimatedNumber
                        value={derivedParams.orbitalPeriod}
                        decimals={2}
                        suffix=" years"
                      />
                    </p>
                ...
                <div className="space-y-3">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <Tooltip>
                        <TooltipTrigger>
                          <Info className="h-3 w-3 text-gray-400" />
                          <TooltipContent className="max-w-xs w-60 whitespace-normal break-words">
                            Average surface temperature. Decreases with orbital
                            distance squared. Affected by stellar mass
                            (luminosity) and atmospheric greenhouse effects.
                            Critical for liquid water
                          </TooltipContent>
                        </TooltipTrigger>
                      </Tooltip>
                    </div>
                    <p className="font-mono text-blue-400">
                      <AnimatedNumber
                        value={derivedParams.temperatureK}
                        decimals={0}
                        suffix=" K"
                      />
                    </p>
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <Tooltip>
                        <TooltipTrigger>
                          <Info className="h-3 w-3 text-gray-400" />
                          <TooltipContent className="max-w-xs w-60 whitespace-normal break-words">
                            Weight of atmosphere pressing down. Depends on
                            planet mass (gravity), atmospheric composition, and
                            temperature. Earth: 1 bar. Affects liquid water
                            stability
                          </TooltipContent>
                        </TooltipTrigger>
                      </Tooltip>
                    ...
                    <p className="font-mono text-blue-400">
                      <AnimatedNumber
                        value={derivedParams.atmosphericPressure}
                        decimals={2}
                        suffix=" atm"
                      />
                    </p>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Tooltip>
                        <TooltipTrigger>
                          <Info className="h-3 w-3 text-gray-400" />
                          <TooltipContent className="max-w-xs w-60 whitespace-normal break-words">
                            Gravitational pull at surface. Scales as
                            Mass/Radius². Doubling mass doubles gravity;
                            doubling radius quarters it. Affects atmospheric
                            retention and escape velocity
                          </TooltipContent>
                        </TooltipTrigger>
                      </Tooltip>
                    </div>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2 text-xs">
                  {derivedParams.temperatureK >= 273 &&
                    derivedParams.temperatureK <= 373 && (
                      <div className="flex items-center text-green-400">
                        <div className="w-2 h-2 bg-green-400 rounded-full mr-2" />
                        Liquid water possible
                      </div>
                    )}
                  {derivedParams.atmosphericPressure >= 0.1 &&
                    derivedParams.atmosphericPressure <= 10 && (
                      <div className="flex items-center text-green-400">
                        <div className="w-2 h-2 bg-green-400 rounded-full mr-2" />
